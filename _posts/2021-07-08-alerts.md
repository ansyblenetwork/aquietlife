...
function update() {
	...
	if (buffer) {
		...
		buffer.forEach(function(stockData) {
			...
			volume[stockData.symbol].shift();			  
			volume[stockData.symbol].push(regularMarketVolume);
			price[stockData.symbol].shift();			  
			price[stockData.symbol].push(regularMarketPrice);
		});	
