...
function updateDisplay(stock, fixed) {			
	...
	if (fixed == lpelt || fixed == upelt) {				
		...
		upper.value = round(val(upper) - val(lower) + newLower);
	}	 
	...
	if (data.quoteResponse.result) {
		...
		data.quoteResponse.result.forEach(function(stockData) {
			...
			if (volumes[stock][volumes[stock].length - 1] != volume ||
				...
				if (avgPrice > 0 && curPrice > 0 && (curPrice > 3*avgPrice || curPrice + 3*avgPrice < 0)) activityNotification("price");
				if (avgVol > 0 && curVol > 0  && (curVol > 3*avgVol || curVol + 3*avgVol < 0)) activityNotification("volume");
				if (notify) {
					if (curPrice > 0) D(stock + "-percent").style.backgroundColor = "#efe";
					else D(stock + "-percent").style.backgroundColor = "#fee";   
