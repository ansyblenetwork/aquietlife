...
id="optionsInput" type="text" style="margin:10px;" autocomplete="off" placeholder="Options chain..."><button onclick="optionsChain()">Fetch</button>
	...
	<div id="optionsPrice" style="font-weight:bold; text-align:center"></div>
...
function checkDefData(defData, altData, data) {      
	for (let contractStrike in data) {	
		...
		if (contract.ask > 0) {	
			if (defData.premium) {
				if (contract.ask < defData.premium && dstrike < newstrike
					&& (altData.premium == 0 || defData.premium < altData.premium || (defData.premium == altData.premium && dstrike < astrike))) {
					altData.strike = defData.strike;
					altData.premium = defData.premium;
					altData.strikeName = defData.strikeName;	
				} else if (contract.ask > defData.premium && newstrike < dstrike
					&& (altData.premium == 0 || contract.ask < altData.premium || (contract.ask == altData.premium && newstrike < astrike))) {
...
function returnRate(contract, defData) {
	...
	if (contract.putCall == "CALL")	return Math.round(100*(contract.bid - defData.premium)*365*1000*60*60*24
			/((defData.strike - contract.strikePrice)*(1000*60*60*48 + contract.expirationDate-Date.now())));
	else return Math.round(100*(contract.bid - defData.premium)*365*1000*60*60*24
			/((contract.strikePrice - defData.strike)*(1000*60*60*48 + contract.expirationDate-Date.now())));
}
	
function marginThreshold(contract) {
	if (contract.daysToExpiration > 2) {
		if (contract.putCall == "CALL") return 10;
		else return 5;
	} else return 0;
}

function margin(contract, data) {
	if (contract.putCall == "CALL") return Math.round(100*(contract.strikePrice - data.underlyingPrice)/data.underlyingPrice);
	else return Math.round(100*(data.underlyingPrice - contract.strikePrice)/data.underlyingPrice);
}
							       
function optionsChain() {
	fetch("https://ansyble.herokuapp.com/cors/", 
		{cache:'no-cache', headers: {'Target-URL': 'https://query1.finance.yahoo.com/v7/finance/quote?symbols=' 
								+ D('optionsInput').value.toUpperCase() }}).then(function(response) {
		...
		if (data.quoteResponse.result) {	
			data.quoteResponse.result.forEach(function(stockData) {
				...
				D('optionsPrice').textContent = stock + " " + round(daily) + " (" + round(dailyPercent, true) + "%)";

				if (dailyPercent > 0) D('optionsPrice').style.color = "#3b7";
				else if (dailyPercent < 0) D('optionsPrice').style.color = "#F00";
				else D('optionsPrice').style.color = "#333";
			});	
		} else {
			var e = new Date();
	...
	fetch("https://api.tdameritrade.com/v1/marketdata/chains?apikey=T1V8GYUYK3GKC7HG3L23O9XBJ5OH1C4F&symbol="
		...
		if (data.status == "SUCCESS") {
			...
			function buildli(contract, margin, returnrate, returnrate2, defleg, defprem) {
				...
				let colstring = returnrate + '/' + returnrate2 + "%";
				if (returnrate2 > returnrate) colstring += " (" + returnrate2 + "%)";
				else colstring += " (" + returnrate + "%)";
				let col = addCol(colstring, "150px", li);
	
	
				let rate = 295 - 2*returnrate;
				if (returnrate2 > returnrate) rate = 295 - 2*returnrate2; 
			...
			/////////////////////// CALLS //////////////////////////////////
				...
				let calldefleg = referenceli;
				let calldefleg2;
				if (altData.strikeName) {
					referenceli = buildDefense(data.callExpDateMap[contractDate][altData.strikeName][0]);
					calldefleg2 = referenceli;
				}
				...
				for (let contractStrike in data.callExpDateMap[contractDate]) {
					...
					if (margin(contract, data) >  marginThreshold(contract)) {
						...
						let theleg = calldefleg;
						let prem = defData.premium;
						if (returnrate2 > returnrate) {
							theleg = calldefleg2;
							prem = altData.premium;
						}

						if (returnrate > 19 || returnrate2 > 19) {
							if (!referenceli.nextElementSibling) 
								D('optionslist').appendChild(buildli(contract, margin(contract, data), 
									returnrate, returnrate2, theleg, prem));
							else D('optionslist').insertBefore(buildli(contract, margin(contract, data), 
									returnrate, returnrate2, theleg, prem), 
			...
			/////////////////////// MIDDLE //////////////////////////////////
				...
				addCol(data.symbol + " " + data.callExpDateMap[contractDate][defData.strikeName][0].daysToExpiration + " DTE", "600px", referenceli);
			...
			/////////////////////// PUTS //////////////////////////////////
				...
				let putdefleg2;
				if (altData.strikeName) putdefleg2 = buildDefense(data.putExpDateMap[contractDate][altData.strikeName][0]);
...
function mergeAll() {
			...
			/////////////////////// CALLS //////////////////////////////////
				...
				for (let contractStrike in data.callExpDateMap[contractDate]) {
					...
					if (margin(contract, data) > marginThreshold(contract)) {
						...
						if (returnrate > 19 || returnrate2 > 19) {
							if (returnrate > maxdata[i].call) maxdata[i].call = returnrate;
			...
			/////////////////////// PUTS //////////////////////////////////
				...
				for (let contractStrike in data.putExpDateMap[contractDate]) {
					...
					if(margin(contract, data) > marginThreshold(contract)) {
						...
						if (returnrate > 19 || returnrate2 > 19) {
							if (returnrate > maxdata[i].put) maxdata[i].put = returnrate;
	...
	Promise.all(promises).then(function() {
		...
		for (let i = 0; i < maxdata.length; i++) {
			mergeString += `<strong style="cursor:pointer" onclick="D('optionsInput').value = '${maxdata[i].symbol}'; optionsChain()" style='color:#333'>${maxdata[i].symbol}</strong> `;
		}
		...
		if (data.quoteResponse.result) {
			...
			data.quoteResponse.result.forEach(function(stockData) {
				...
				(stockData.earningsTimestampStart == stockData.earningsTimestampEnd && stockData.earningsTimestamp - Date.now()/1000 < 2*24*60*60)) {
					...
					console.log("Earnings for: " + stockData.symbol);
					if (stockData.earningsTimestamp - Date.now()/1000 < 0 && stockData.earningsTimestamp - Date.now()/1000 > -2*24*60*60)
						console.log("because first true: " + stockData.earningsTimestamp);
					if (stockData.earningsTimestampStart == stockData.earningsTimestampEnd && stockData.earningsTimestamp - Date.now()/1000 < 2*24*60*60)
						console.log("because second true: " + stockData.earningsTimestampStart);
				}
			});	      
			...
			earnings.forEach(function(stock) {
				if (earningsTime[stock] - Date.now()/1000 > -2*24*60*60) earningsString += "<a title='" + earningsName[stock] 
				...
				+ "' target='_blank' href='https://finance.yahoo.com/quote/" + stock + "'><strong style='color:blue'>" + stock + "</strong></a> ";							      
			});
