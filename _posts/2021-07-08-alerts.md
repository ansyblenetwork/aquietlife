...
function updateSlider(stock, fixed) {
	...
	if (fixed) {	
		let newLower = Math.round(current - (slider.value * (upper - lower) / 100));
		if (newLower < 0) newLower = 0;
		D(stock + "-lower").value = newLower;
		D(stock + "-upper").value = upper - lower + newLower;
	} else {
		if (upper > lower) {
...
function updatePercent(stock, fixed) {			
	...
	if (fixed) {				
		...
		if (lpelt != document.activeElement) lpelt.value = round(100*(current - parseFloat(lower.value))/current);
		if (upelt != document.activeElement) upelt.value = round(100*(parseFloat(upper.value) - current)/current);
	}
}
...
function submitTicker() {
	...
	if (buffer) {	
		...
		if (!tickerList.includes(stock)) {			
			...
			slider.step = 1;
			slider.value = 50;
			slider.onchange = function() {  updateSlider(stock, slider); updatePercent(stock);  };
