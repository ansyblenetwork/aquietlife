...
function updatePercent(stock, fixed) {			
	...
	} else lower.style.fontWeight = "initial";
	if (parseFloat(upelt.value) < 0) {
	...
	} else upper.style.fontWeight = "initial";
}
	...
	if (buffer) {
		...
		buffer.forEach(function(stockData) {
			...
			if (volumes[stock][volumes[stock].length - 1] != volume ||
				...
				let notify = false;
				if (avgPrice > 0 && curPrice > 0 && (curPrice > 2*avgPrice || curPrice + 2*avgPrice < 0)) {
				...
				if (avgVol > 0 && curVol > 0  && (curVol > 2*avgVol || curVol + 2*avgVol < 0)) {	
					if (D(stock + "-percent").style.fontWeight != "bold") {
						if (active(D('soundButton'))) {
							if (curPrice > 0) D('audio').play();
							else D('audiodown').play();
