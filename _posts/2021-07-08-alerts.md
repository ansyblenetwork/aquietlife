		...
		if (data.status == "SUCCESS") {
			...
			for (let contractDate in data.callExpDateMap) {
				...
				for (let contractStrike in data.callExpDateMap[contractDate]) {
					...
					if (dpremium == 0 || contract.bid < dpremium || (contract.bid == dpremium && contract.strikePrice < dstrike) ) {
						dstrike = contract.strikePrice;
						dpremium = contract.bid;
						theStrike = contractStrike;
					}	       
								       
					if (contract.bid > 0 && lastTrade < 12*60
							&& returnrate > 14 && margin > 14) {
