		...
		if (data.status == "SUCCESS") {
			...
			/////////////////////// CALLS //////////////////////////////////
				...
				for (let contractStrike in data.callExpDateMap[contractDate]) {
					...
					returnrate = callRate(contract, defData);
					returnrate2 =callRate(contract, altData);
			...
			/////////////////////// PUTS //////////////////////////////////
				...
				for (let contractStrike in data.putExpDateMap[contractDate]) {
					...
					returnrate = putRate(contract, defData);
					returnrate2 = putRate(contract, altData);
...
function callRate(contract, defData) {
	return Math.round(100*(contract.bid - defData.premium)*365*1000*60*60*24
			/((defData.strike - contract.strikePrice)*(contract.expirationDate-Date.now())));
}
	
function putRate(contract, defData) {
	return Math.round(100*(contract.bid - defData.premium)*365*1000*60*60*24
