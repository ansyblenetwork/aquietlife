...
function analyze() {	
		...
		if (data.status == "SUCCESS") {
			...
			/////////////////////// CALLS //////////////////////////////////
				...
				for (let contractStrike in data.callExpDateMap[contractDate]) {
					...
					if (contract.bid > defData.premium && lastTrade < 24*60 && returnrate > 19 && margin > threshold) {
	...
	Promise.all(promises).then(function() {
		...
		for (let i = 0; i < maxdata.length; i++) {
			mergeString += "<strong  style='color:#333'>" + maxdata[i].symbol + "</strong> ";
		}
...
function updateStocktwits() {
	...
	fetch("https://ansyble.herokuapp.com/cors/", 
		...
		data.symbols.forEach(function(stockData) {
			stString += printWeekly(stockData.symbol, stList, stWeeklyList, prevWeekly, "Stocktwits");
		});
...
function printWeekly(symbol, stList, stWeeklyList, prevWeekly, "Stocktwits") {
	if (monthlys.includes(symbol) || weeklys.includes(symbol)) {
		...
		if (weeklys.includes(symbol)) {
			...
			if (prevWeekly.length > 0 && !prevWeekly.includes(symbol)) {
				...
				return stLinkify(<strong style='color:blue'>" + symbol + "</strong></a>") + " ";
			} else return stLinkify(<strong style='color:#333'>" + symbol + "</strong></a>") + " ";
		} else return stLinkify(symbol) + " ";
	} else return "";
}	
	
function stLinkify(str, stock) {
	return <a target='_blank' href='https://stocktwits.com/symbol/" + stock + "'>" + str + "</a>";
}
