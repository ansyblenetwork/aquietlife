...
function update() {
	...
	if (buffer) {
		...
		buffer.forEach(function(stockData) {
			...
			if (volumes[stockData.symbol][volumes[stockData.symbol].length - 1] != stockData.regularMarketVolume ||
			    prices[stockData.symbol][prices[stockData.symbol].length - 1] != stockData.regularMarketPrice) {
				...
				if (direction && D('notificationButton').textContent == "Notifications On") {
					new Notification(stockData.symbol + "'s " + direction + " activity is unusually high.");
