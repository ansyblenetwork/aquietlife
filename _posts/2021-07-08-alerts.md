...
function contractAboveVolatility(contract, symbol) {
	let weeksToExp = (1 + (contract.expirationDate-Date.now())/(1000*60*60*24))/7;
	return (optionsQuotes[symbol] && Math.sqrt(weeksToExp) * optionsQuotes[symbol].volatility.split('% ')[0] * 2 < margin(contract, data));
}
...
function generateRec(symbol) {
	...
	/////////////////////// CALLS //////////////////////////////////
		...
		for (let contractStrike in data.callExpDateMap[contractDate]) {
			...
			if (margin(contract, data) >  marginThreshold(contract) 
				&& returnRate(contract, altData) > 19 
				&& contractAboveVolatility(contract, symbol)) callArray.unshift(contract);
		}	       
	...
	/////////////////////// PUTS //////////////////////////////////
		...
		for (let contractStrike in data.putExpDateMap[contractDate]) {
			...
			if (margin(contract, data) >  marginThreshold(contract)
				&& returnRate(contract, altData) > 19 
				&& contractAboveVolatility(contract, symbol)) putArray.push(contract);
		}
...
function updateOptions(newOption) {	
	...
	fetch("https://api.tdameritrade.com/v1/marketdata/chains?apikey=T1V8GYUYK3GKC7HG3L23O9XBJ5OH1C4F&symbol=" 
		...
		if (data.status == "SUCCESS") {	
			...
			function buildli(contract, margin, returnrate, returnrate2, defleg, altData) {
				let li = make('li');
				...
				if (altData) {
					...
					col.title = margin;
					if (contractAboveVolatility(contract, currentOption))  {	
			...
			/////////////////////// CALLS //////////////////////////////////
				...
				callArray.forEach(function(contract) {
					...
					if (maxReturn < returnrate2) {
						maxReturn = returnrate2;
						D('optionslist').appendChild(buildli(contract, margin(contract, data), returnrate, returnrate2, calldefleg2, altData));
					}
				});
			...
			/////////////////////// PUTS //////////////////////////////////
				...
				putArray.forEach(function(contract) {
					...
					if (maxReturn < returnrate2) {
						maxReturn = returnrate2;
						D('optionslist').insertBefore(buildli(contract, margin(contract, data), returnrate, returnrate2, putdefleg2, altData), 
