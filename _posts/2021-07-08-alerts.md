...
function matchPercent(stock, safeObj) {
	let current = parseFloat(D(stock + "-current").value);
	
	if (D(stock + "-lowerPercent") != safeObj) D(stock + "-lowerPercent").value = round(100*( current - parseFloat(D(stock + "-lower").value) )/current);
	if (D(stock + "-upperPercent") != safeObj) D(stock + "-upperPercent").value = round(100*( parseFloat(D(stock + "-upper").value) - current )/current);
	if (safeObj) {
		D(stock + "-lower").value = round( current * (1 - parseFloat(D(stock + "-lowerPercent").value)/100));
		D(stock + "-upper").value = round( current * (1 + parseFloat(D(stock + "-upperPercent").value)/100));
