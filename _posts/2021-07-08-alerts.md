		...
		for (let contractStrike in data.callExpDateMap[contractDate]) {
			...
			if (margin(contract, data) >  marginThreshold(contract)) {
				...
				if (returnrate2 > 19) {
					...
					if (optionsQuotes[currentOption] && Math.sqrt(weeksToExp) * optionsQuotes[currentOption].volatility.split('% ')[0] * 2 
		...
		if (data.status == "SUCCESS") {	
			...
			function buildli(contract, margin, returnrate, returnrate2, defleg, altData) {
				...
				if (altData) {
					...
					col.title = margin;
					if (optionsQuotes[currentOption] && Math.sqrt(weeksToExp) * optionsQuotes[currentOption].volatility.split('% ')[0] * 2 < margin)  {	
