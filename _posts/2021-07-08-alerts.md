...
function matchPercent(ticker, safeObj) {
	...
	if (safeObj) {
		D(ticker + "-lower").value = round( current * (1 - parseFloat(D(ticker + "-lowerPercent").value)/100));
		D(ticker + "-upper").value = round( current * (1 + parseFloat(D(ticker + "-upperPercent").value)/100));
...
function submitTicker() {
	...
	if (buffer) {	
		...
		if (!tickerList.includes(stock)) {			
			...
			lowerBound.onblur = function() { matchPercent(stock); update() };
			...
			upperBound.onblur = function() {  matchPercent(stock); update() };
