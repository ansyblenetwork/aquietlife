...
function matchPercent(stock, fixed) {
	...
	function updateSlider() {		     
		if (slider != fixed && slider != document.activeElement) {
			if (upper > lower) {
	...
	if (fixed) {
		lower = round( current * (1 - parseFloat(lpelt.value)/100));
		upper = round( current * (1 + parseFloat(upelt.value)/100));
		D(stock + "-lower").value = lower;
		D(stock + "-upper").value = upper;
		fixed.value = round(parseFloat(fixed.value));
		updateSlider();
	} else {
		if (lpelt != document.activeElement) lpelt.value = round(100*(current - lower)/current);
		if (upelt != document.activeElement) upelt.value = round(100*(upper - current)/current);
		updateSlider();
	}
