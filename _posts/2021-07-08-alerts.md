...
function updateMargin(stock, fixed) {	
	let current = parseFloat(D(stock + "-current").textContent);
	let upper = parseFloat(D(stock + "-upper").value);
	let lower = parseFloat(D(stock + "-lower").value);
	...
	if (fixed) {	
		let newLower = round(current - parseFloat(lowerMargin.value));
		if (newLower < 0) newLower = 0;
		D(stock + "-lower").value = newLower;
	
		let newUpper = round(current + parseFloat(upperMargin.value));
		if (newUpper < 0) newUpper = 0;
		D(stock + "-upper").value = newUpper;
				 
		updatePercent(stock);
	} else {
		upperMargin.value = round(upper - current);
		lowerMargin.value = round(current - lower);
	}				 
}
...
function submitTicker() {
	...
	for (let i = bufferList.length - 1; i > -1; i--) {
		...
		if (!tickerList.includes(stock)) {			
			...
			lowerMargin.onblur = function() { updateMargin(stock, lowerMargin); };
			setInput(lowerMargin);
			lowerMargin.style.paddingLeft = "10px";  	
			...
			upperMargin.onblur = function() { updateMargin(stock, upperMargin); };
			setInput(upperMargin);
			upperMargin.style.paddingRight = "10px";  	
			...
			updatePercent(stock, upperPercent);
