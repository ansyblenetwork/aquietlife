		...
		if (data.status == "SUCCESS") {
			...
			for (let contractDate in data.callExpDateMap) {
				for (let contractStrike in data.callExpDateMap[contractDate]) {
					...
					addCol(Math.round((Date.now() - contract.tradeTimeInLong)/60000), "50px");
								       
					if (contract.putCall == "CALL") addCol(Math.round(100*(contract.strikePrice - contract.underlyingPrice)/contract.underlyingPrice) + "%", "100px");
					else addCol(Math.round(100*(contract.underlyingPrice - contract.strikePrice)/contract.underlyingPrice) + "%", "100px");
					addCol(Math.round(100*contract.bid * 365 /(contract.strikePrice * contract.daysToExpiration)) + "%", "100px");
