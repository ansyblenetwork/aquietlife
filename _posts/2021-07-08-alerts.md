...
<div style="float:right">
	...
	<button onclick="update(); updateStocktwits();">Refresh</button>
...
<button id="stButton" style="margin:0px auto 20px auto; display:block;" onclick="addWeeklys();">Load</button>
	
<div style="margin-bottom:10px; text-align:center" id="yolo">[Trending tickers on WSB with <strong>weeklys</strong>]</div>
<button id="yoloButton" style="margin:0px auto 20px auto; display:block;" onclick="addYolo();">Load</button>
...
function addWeeklys() {
	if (D('stButton').textContent == "Load") updateStocktwits();
		...
		D('tickerInput').value = weeklyList.join(); 
		submitTicker();
	}
}
...
function addYolo() {	
	...
	fetch(query, {cache:'no-cache'}).then(function(response) {
	...
	fetch("https://ansyble.herokuapp.com/cors/", 
		...
		data.symbols.forEach(function(stockData) {
			...
			if (weeklys.includes(stockData.symbol)) {
				...
				if (prevWeekly.length > 0 && !prevWeekly.includes(stockData.symbol)) {
					if (active(D('soundButton'))) D('audio').play();
					if (active(D('notificationButton'))) new Notification(stockData.symbol + " is newly trending.");
				}
