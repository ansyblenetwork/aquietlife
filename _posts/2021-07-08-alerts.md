		...
		if (data.status == "SUCCESS") {
			...
			for (let contractDate in data.callExpDateMap) {
				for (let contractStrike in data.callExpDateMap[contractDate]) {
					...
					addCol(contract.description, "250px");
					...
					let diff = Math.round((Date.now() - contract.tradeTimeInLong)/60000);
					addCol(diff, "50px");
					addCol(contract.daysToExpiration, "50px");
					addCol(round(100*contract.last * 365 /(contract.strikePrice * contract.daysToExpiration), true) + "%", "50px");
