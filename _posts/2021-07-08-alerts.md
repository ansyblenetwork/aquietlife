...
function matchPercent(stock, safeObj) {
	...
	if (D(stock + "-lowerPercent") != safeObj) D(stock + "-lowerPercent").value = round(100*( current - lower) )/current);
	if (D(stock + "-upperPercent") != safeObj) D(stock + "-upperPercent").value = round(100*( upper - current )/current);
	if (safeObj) {
