...
function update() {
	...
	if (buffer) {
		...
		buffer.forEach(function(stockData) {
			...
			D(stockData.symbol + "-percent").textContent = stockData.symbol + " " + round(buffer.regularMarketChangePercent) + "%";
	
			if (buffer.regularMarketChangePercent > 0) D(stockData.symbol + "-percent").style.color = "#00FF00";
			else if (buffer.regularMarketChangePercent < 0) D(stockData.symbol + "-percent").style.color = "#FF0000";
	...
	if (buffer) {	
		...
		if (!tickerList.includes(stock)) {			
			...
			newTicker.style.display = "inline-block";	
				...
				newTicker.style.width = "100%";
			if (buffer.regularMarketChangePercent > 0) newTicker.style.color = "#00FF00";
