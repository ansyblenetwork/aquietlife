...
A three-part stock alert applet:

1. Discovers active stocks from screeners and social media sites.
2. Analyzes options chains to select volatility-adjusted OTM contracts.
3. Monitors recently viewed stocks for price movements beyond a specified range and elevated price/volume activity.



<div style="min-width:650px; padding: 30px 0px 30px 0px">

	<div style="overflow:auto; max-height:50vh; margin-bottom:20px">
...
<div id="controlPanel" style="position:fixed; bottom:10px; left:0px; max-width:920px;">
...
<div style="float:right">
	<span id="msg" style="margin-right:10px; border-radius:5px; background-color:white; padding: 5px 10px;"></span>
...
<ul style="overflow:auto; max-height:50vh; list-style-type:none; padding-left:0px;padding-bottom:30px; margin-bottom:20px; border-bottom:solid" id="recList"></ul>
...
<ul style="overflow:auto; max-height:50vh; list-style-type:none; padding-left:0px;padding-bottom:30px; margin-bottom:20px; border-bottom:solid" id="optionslist"></ul>
...
function generateRec(symbol) {
	if (recCount == 0) D('recList').textContent = "";
	recCount++;
	let data = optionsData[symbol];
	
	recData[symbol] = {data: data, expirations:[], expirationDates:[]};
	...
	/////////////////////// PUTS //////////////////////////////////
		...
		if (candidate) {
			candidateObj.put = {contract:candidate, altData:altData};
		}
		
		recData[symbol].expirationDates.push(defData.contract.daysToExpiration);
		recData[symbol].expirations.push(candidateObj);	
		buildRec(symbol, candidateObj, defData.contract.daysToExpiration);	
		sortRecData.push({symbol:symbol, candidateObj:candidateObj, daysToExpiration: defData.contract.daysToExpiration });
	}
	if (recCount == merge.length) {
		D('recList').textContent = "";
	//	for (let sym in recData) {
	//		for (let i = 0; i < recData[sym].expirations.length; i++) {
	//			buildRec(sym, recData[sym].expirations[i], recData[sym].expirationDates[i]);
	//		}
	//	}
		sortRecData.sort(function(x, y) {
			let xcall = 0;
			let xput = 0;
			let ycall = 0;
			let yput = 0;
			if (x.candidateObj.call) xcall = returnRate(x.candidateObj.call.contract, x.candidateObj.call.altData);
			if (x.candidateObj.put) xput = returnRate(x.candidateObj.put.contract, x.candidateObj.put.altData);
			if (y.candidateObj.call) ycall = returnRate(y.candidateObj.call.contract, y.candidateObj.call.altData);
			if (y.candidateObj.put) yput = returnRate(y.candidateObj.put.contract, y.candidateObj.put.altData);
			return ycall + yput - xcall - xput;
		});
		for (let i = 0; i < sortRecData.length; i++) {
			buildRec(sortRecData[i].symbol, sortRecData[i].candidateObj, sortRecData[i].daysToExpiration);
		}
	}
}
							    
function buildRec(symbol, candidateObj, exp) {
						    
	////////////// build the table /////////////////////////////////						    
							    
		if (candidateObj.call) {
			let calldefleg2 = buildli(candidateObj.call.altData.contract, recData[symbol].data);
			D('recList').appendChild(calldefleg2);
			D('recList').appendChild(buildli(candidateObj.call.contract, recData[symbol].data, calldefleg2, candidateObj.call.altData));
		}
	...
	/////////////////////// MIDDLE //////////////////////////////////
		...
		addCol("$" + round(recData[symbol].data.underlyingPrice), "75px", referenceli);
		addCol(recData[symbol].data.symbol + " " + exp + " DTE", "675px", referenceli);
		D('recList').appendChild(referenceli);	

	/////////////////////// PUTS //////////////////////////////////

		if (candidateObj.put) {
			let putdefleg2 = buildli(candidateObj.put.altData.contract, recData[symbol].data);
			D('recList').appendChild(putdefleg2);
			putdefleg2.style.marginBottom = "30px";
			D('recList').insertBefore(buildli(candidateObj.put.contract, recData[symbol].data, putdefleg2, candidateObj.put.altData), referenceli.nextElementSibling);			    
		} else referenceli.style.marginBottom = "30px";					    
							    
}
	
function setDefLeg(defData, altData, data) {      
...
function evalContract(contract, data, altData) {
	return (margin(contract, data) > marginThreshold(contract) && returnRate(contract, altData) > 19);
}
		...
		if (data.status == "SUCCESS") {	
			...
			/////////////////////// CALLS //////////////////////////////////
				...
				callArray.forEach(function(contract) {				
					if (maxReturn < returnRate(contract, altData)) {
						maxReturn = returnRate(contract, altData);
						D('optionslist').appendChild(buildli(contract, data, calldefleg2, altData));
					}
				});
			...
			/////////////////////// PUTS //////////////////////////////////
				...
				putArray.forEach(function(contract) {				
					if (maxReturn < returnRate(contract, altData)) {
						maxReturn = returnRate(contract, altData);
						D('optionslist').insertBefore(buildli(contract, data, putdefleg2, altData), referenceli.nextElementSibling);
					}
				});
			}
...
function mergeAll() {
	...
	fetch("https://ansyble.herokuapp.com/cors/", 
		{cache:'no-cache', headers: {'Target-Ticker': merge[i] }}).then(function(response) {
		return response.json();
	}).then(function(data) {
		optionsQuotes[merge[i]] = data;
		if (optionsData[merge[i]]) generateRec(merge[i]);
	});
	...
	fetch("https://api.tdameritrade.com/v1/marketdata/chains?apikey=T1V8GYUYK3GKC7HG3L23O9XBJ5OH1C4F&symbol="
		...
		console.log(data);
		if (data.status == "SUCCESS") {
			optionsData[merge[i]] = data;
			if (optionsQuotes[merge[i]]) generateRec(merge[i]);
			...
			/////////////////////// CALLS //////////////////////////////////
				...
				for (let contractStrike in data.callExpDateMap[contractDate]) {
					let contract = data.callExpDateMap[contractDate][contractStrike][0];
					if (evalContract(contract, data, altData)
						&& returnRate(contract, altData) > maxdata[i].call) maxdata[i].call = returnRate(contract, altData);
				}
			...
			/////////////////////// PUTS //////////////////////////////////
				...
				for (let contractStrike in data.putExpDateMap[contractDate]) {
					...
					if(evalContract(contract, data, altData)
						&& returnRate(contract, altData) > maxdata[i].put) maxdata[i].put = returnRate(contract, altData);
				}
			}
		}					       	       
	})
	...
	Promise.all(promises).then(function() {
		...
		for (let i = 0; i < maxdata.length; i++) {
			...
			mergeString += `<strong style="cursor:pointer; color:#94b" onclick="D('optionsInput').value = '${maxdata[i].symbol}'; optionsChain()" style='color:#333'>${maxdata[i].symbol}</strong> `;
	...
	fetch("https://ansyble.herokuapp.com/cors/", 
		...
		if (data.quoteResponse.result) {
			...
			earnings.forEach(function(stock) {
				...
				+ "' target='_blank' href='https://finance.yahoo.com/quote/" + stock + "'><strong style='color:#94b'>" + stock + "</strong></a> ";							      
			});
...
1. Alerts are tuned for events on the intra-day scale.
2. Quote data comes from Yahoo Finance, options data is from TD Ameritrade, and both are real-time. Auto-refresh is per 10 seconds.
3. "Elevated price/volume activity" is measured on the scale of minutes. Precisely, a notification occurs when a quote has four times the average volume or price over roughly the last two minutes of data.
