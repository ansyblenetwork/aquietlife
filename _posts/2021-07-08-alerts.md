...
<div style="min-width:650px; padding: 40px 20px 40px 0px">

<div style="position:fixed; bottom: 10px; background-color:white">
<button onclick="update(); updateAll();" >Refresh</button>
	<span id="msg2" style="padding-left:10px; margin-right:10px;"></span>
	</div>
	
<div style="margin-bottom:20px; text-align:center">
	<a target='_blank' href='https://finviz.com/screener.ashx?v=150&f=sh_opt_option&o=-change'><strong style='color:#333'>Finviz</strong></a>
	<br>
	<a target='_blank' href='https://finviz.com/screener.ashx?v=150&f=sh_opt_option&o=-change' style="margin-right:10px"><strong style='color:#333'>Gainers</strong></a>
	<a target='_blank' href='https://finviz.com/screener.ashx?v=150&f=sh_opt_option&o=-change' style="margin-right:10px" id="finvizgainers"><strong style='color:#333'>Gainers with weeklys</strong></a>
	<a target='_blank' href='https://finviz.com/screener.ashx?v=150&f=sh_opt_option&o=change' style="margin-right:10px"><strong style='color:#333'>Losers</strong></a>
	<a target='_blank' href='https://finviz.com/screener.ashx?v=150&f=sh_opt_option&o=change' id="finvizlosers"><strong style='color:#333'>Losers with weeklys</strong></a>
</div>
	
<div style="margin-bottom:10px; text-align:center" id="stocktwits">Trending on <a target='_blank' href='https://stocktwits.com/'><strong style='color:#333'>Stocktwits</strong></a> with <strong>weeklys</strong></div>
...
<div style="margin-bottom:10px; text-align:center" id="yolo">Trending on <a target='_blank' href='https://yolostocks.live/'><strong style='color:#333'>YoloStocks</strong></a> with <strong>weeklys</strong></div>
...
<div style="margin-bottom:10px; text-align:center" id="wsb">Trending on <a target='_blank' href='https://dashboard.nbshare.io/apps/reddit/wallstreetbets/'><strong style='color:#333'>WSB</strong></a> with <strong>weeklys</strong></div>
...
<div style="margin-bottom:10px; text-align:center" id="earnings"><a target='_blank' href='https://finance.yahoo.com/calendar/earnings'><strong style='color:#333'>Earnings</strong></a> within a week with <strong>weeklys</strong></div>
...
function mergeAll() {
		...
		if (data.status == "SUCCESS") {
			...
			/////////////////////// CALLS //////////////////////////////////
				...
				for (let contractStrike in data.callExpDateMap[contractDate]) {	
					checkDefData(defData, altData, data.callExpDateMap[contractDate][contractStrike][0], contractStrike);
				}	
				...
				for (let contractStrike in data.callExpDateMap[contractDate]) {
					...
					returnrate = Math.round(100*(contract.bid - defData.premium)*365*1000*60*60*24
							/((defData.strike - contract.strikePrice)*(contract.expirationDate-Date.now())));
					returnrate2 = Math.round(100*(contract.bid - altData.premium)*365*1000*60*60*24
							/((altData.strike - contract.strikePrice)*(contract.expirationDate-Date.now())));
					...
					if (contract.bid > defData.premium && lastTrade < 72*60 && returnrate > 19 && margin > threshold) {
						if (returnrate2 > maxdata[i].call) maxdata[i].call = returnrate2;
					}
				}
			...
			/////////////////////// PUTS //////////////////////////////////
				...
				for (let contractStrike in data.putExpDateMap[contractDate]) {	
					checkDefData(defData, altData, data.putExpDateMap[contractDate][contractStrike][0], contractStrike);
				}	
				if (altData.strike <= defData.strike) altData = {strike:0, premium:0, strikeName:""};
								       
				for (let contractStrike in data.putExpDateMap[contractDate]) {
					contract = data.putExpDateMap[contractDate][contractStrike][0];
					if (contract.tradeTimeInLong) lastTrade = Math.round((Date.now() - contract.tradeTimeInLong)/60000);
					returnrate = Math.round(100*(contract.bid - defData.premium)*365*1000*60*60*24
						/((contract.strikePrice - defData.strike)*(contract.expirationDate-Date.now())));
					returnrate2 = Math.round(100*(contract.bid - altData.premium)*365*1000*60*60*24
						/((contract.strikePrice - altData.strike)*(contract.expirationDate-Date.now())));
					margin = Math.round(100*(data.underlyingPrice - contract.strikePrice)/data.underlyingPrice);
								  
					let threshold = 5;
					if (contract.daysToExpiration < 3) threshold = 0;  
					if (contract.bid > defData.premium && lastTrade < 72*60 && returnrate > 19 && margin > threshold) {	
						if (returnrate2 > maxdata[i].put) maxdata[i].put = returnrate2;
					}
				}
			}
		}					       
	...
	Promise.all(promises).then(function() {
		...
		for (let i = 0; i < maxdata.length; i++) {
			mergeString += `<strong style="cursor:pointer" onclick="addOption('${maxdata[i].symbol}')" style='color:#333'>${maxdata[i].symbol}</strong> `;
		}
...
function checkDefData(defData, altData, contract, contractStrike) {
	let dstrike = defData.strike;
	let astrike = altData.strike;
	let newstrike = contract.strikePrice;
	if (contract.putCall == "PUT") {
		dstrike = -dstrike; astrike = -astrike; newstrike = -newstrike;
	}
	if (contract.ask > 0) {	
		if (defData.premium) {
			if (contract.ask < defData.premium
				&& (altData.premium == 0 || defData.premium < altData.premium || 
					(defData.premium == altData.premium && dstrike < astrike))) {
				altData.strike = defData.strike;
				altData.premium = defData.premium;
				altData.strikeName = defData.strikeName;	
			} else if (contract.ask > defData.premium 
				&& (altData.premium == 0 || contract.ask < altData.premium || 
					(contract.ask == altData.premium && newstrike < astrike))) {
				altData.strike = contract.strikePrice;
				altData.premium = contract.ask;
				altData.strikeName = contractStrike;			       
			}
		}
		if (defData.premium == 0 || contract.ask < defData.premium || 
			(contract.ask == defData.premium && newstrike < dstrike)) {
...
function addOption(stock) {
	D('optionsInput').value = stock; optionsChain();
}
...
function updateEarnings() {
	...
	fetch("https://ansyble.herokuapp.com/cors/", 
		...
		if (data.quoteResponse.result) {
			let earningsTime = {};
			let earningsName = {};
			let earningsString = "<a target='_blank' href='https://finance.yahoo.com/calendar/earnings'><strong style='color:#333'>Earnings</strong></a>: ";
...
function updateYolo() {
	...
	fetch("https://ansyble.herokuapp.com/cors/", 
		...
		yoloWeeklyList = [];
		let yoloString = "<a target='_blank' href='https://yolostocks.live/'><strong style='color:#333'>YoloStocks</strong></a> (" + new Date().toLocaleTimeString() + "): ";
...
function updateWSB() {
	...
	fetch("https://ansyble.herokuapp.com/cors/", 
		...
		wsbWeeklyList = [];
		let wsbString = "<a target='_blank' href='https://dashboard.nbshare.io/apps/reddit/wallstreetbets/'><strong style='color:#333'>WSB</strong></a> (" + new Date().toLocaleTimeString() + "): ";
		for (let i = 0; i < 15; i++) {
...
function updateStocktwits() {
	...
	fetch("https://ansyble.herokuapp.com/cors/", 
		...
		stWeeklyList = [];
		let stString = "<a target='_blank' href='https://stocktwits.com/'><strong style='color:#333'>Stocktwits</strong></a> (" + new Date().toLocaleTimeString() + "): ";
	...
	fetch("https://api.tdameritrade.com/v1/marketdata/chains?apikey=T1V8GYUYK3GKC7HG3L23O9XBJ5OH1C4F&symbol="
		...
		if (data.status == "SUCCESS") {
			...
			function buildDefense(contract) {	
				...
				if (rate < 0) myli.children[4].style.backgroundColor = "#00ffff";
				else if (rate < 16)  myli.children[4].style.backgroundColor = "#0" + rate.toString(16) + "ffff";
				else  myli.children[4].style.backgroundColor = "#" + rate.toString(16) + "ffff";
			...
			function buildli(contract, lastTrade, margin, returnrate, returnrate2) {
				...
				addCol("$" + contract.strikePrice, "75px", li);
				if (contract.putCall == "CALL") addCol("+" + margin + "%", "75px", li);
				else addCol("-" + margin + "%", "75px", li);
			...
			/////////////////////// CALLS //////////////////////////////////
				...
				let referenceli = buildDefense(data.callExpDateMap[contractDate][defData.strikeName][0]);	
				if (altData.strikeName) referenceli = buildDefense(data.callExpDateMap[contractDate][altData.strikeName][0]);
			...
			/////////////////////// MIDDLE //////////////////////////////////
				...
				addCol(data.symbol + " " + data.callExpDateMap[contractDate][defData.strikeName][0].daysToExpiration + " DTE", "550px", referenceli);
			...
			/////////////////////// PUTS //////////////////////////////////
				...
				if (altData.strikeName) buildDefense(data.putExpDateMap[contractDate][altData.strikeName][0]);
				buildDefense(data.putExpDateMap[contractDate][defData.strikeName][0]).style.marginBottom = "30px";
			}
		}					       
		else D('msg2').textContent = "Ticker not found.";		       
	});								       
...
D('finvizgainers').href = "https://finviz.com/screener.ashx?v=150&f=sh_opt_option&o=-change&t=" + weeklys.join();
D('finvizlosers').href = "https://finviz.com/screener.ashx?v=150&f=sh_opt_option&o=change&t=" + weeklys.join();
