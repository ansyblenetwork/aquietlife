	...
	fetch("https://api.tdameritrade.com/v1/marketdata/chains?apikey=T1V8GYUYK3GKC7HG3L23O9XBJ5OH1C4F&symbol="
		...
		if (data.status == "SUCCESS") {
			...
			function buildTitle(contract) {
				...
				addCol("$" + round(data.underlyingPrice), "100px");
				addCol(data.symbol + " " + contract.daysToExpiration + " DTE", "575px");
			...
			function buildli(contract, lastTrade, margin, returnrate, returnrate2) {
				...
				if (contract.putCall == "CALL") addCol("+" + margin + "%", "50px");
				else addCol("-" + margin + "%", "50px");

				let col = addCol(returnrate + '/' + returnrate2 + "%", "100px");
				let rate = 255 - returnrate;
				if (rate < 0) col.style.backgroundColor = "#00dddd";
				else if (rate < 16) col.style.backgroundColor = "#0" + rate.toString(16) + "dddd";
				else col.style.backgroundColor = "#" + rate.toString(16) + "dddd";
			...
			for (let contractDate in data.putExpDateMap) {
				...
				let referenceli = buildTitle(contract);
				...
				if (theaStrike) {
					...
					console.log(theStrike);
				contract = data.putExpDateMap[contractDate][theStrike][0];
				lastTrade = 0;
				if (contract.tradeTimeInLong) lastTrade = Math.round((Date.now() - contract.tradeTimeInLong)/60000);
				returnrate = "x";
				returnrate2 = "x";
