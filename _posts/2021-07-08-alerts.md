...
function update() {
	...
	if (buffer) {
		...
		buffer.forEach(function(stockData) {
			...
			if (avgPrice > 0 && curPrice > 0 && curPrice > 2*avgPrice) D(stockData.symbol + "-percent").style.fontWeight = "bold";
			else D(stockData.symbol + "-percent").style.fontWeight = "initial";
	
			if (avgVol > 0 && curVol > 0  && curVol > 2*avgVol) D(stockData.symbol + "-current").style.fontWeight = "bold";
