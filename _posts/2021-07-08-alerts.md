		...
		if (data.status == "SUCCESS") {
			...
			for (let contractDate in data.callExpDateMap) {
				for (let contractStrike in data.callExpDateMap[contractDate]) {
					...
					if (contract.bid > 0 && returnrate > 14 && lastTrade < 12*60) {
					...
					if (contract.putCall == "CALL") 
						addCol(Math.round(100*(contract.strikePrice - data.underlyingPrice)/data.underlyingPrice) + "%", "50px");
					else addCol(Math.round(100*(data.underlyingPrice - contract.strikePrice)/data.underlyingPrice) + "%", "50px");
