...
function submitTicker() {
	...
	// let buffer = data.optionChain;
	let buffer = data.quoteResponse;
	
	if (buffer.result.length > 0) {	
		let stock = buffer.result[0].underlyingSymbol
		D('ticker').textContent = "Ticker: " + stock;
		D('bid').textContent = "Bid: " + buffer.result[0].quote.bid;
		D('ask').textContent = "Ask: " + buffer.result[0].quote.ask;
		D('market').textContent = "Market: " + buffer.result[0].quote.regularMarketPrice;
