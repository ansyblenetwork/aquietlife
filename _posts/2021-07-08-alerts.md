...
function matchPercent(stock, fixed) {
	...
	if (D(stock + "-lowerPercent") != fixed) D(stock + "-lowerPercent").value = round(100*(current - lower)/current);
	if (D(stock + "-upperPercent") != fixed) D(stock + "-upperPercent").value = round(100*(upper - current)/current);
	if (fixed) {
