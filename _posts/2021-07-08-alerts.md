	...
	fetch("https://api.tdameritrade.com/v1/marketdata/chains?apikey=T1V8GYUYK3GKC7HG3L23O9XBJ5OH1C4F&symbol="
		...
		if (data.status == "SUCCESS") {
			...
			function buildli(contract, margin, returnrate, returnrate2, defleg, defprem) {
				...
				if (returnrate2 > returnrate) colstring += " (" + Math.round(returnrate2) + "%)";
				else colstring += " (" + Math.round(returnrate) + "%)";
				let col = addCol(colstring, "200px", li);
	
	
				let rate = 295 - 2*Math.round(returnrate);
				if (returnrate2 > returnrate) rate = 295 - 2*Math.round(returnrate2); 
				...
				if (contract.askSize > 50*contract.bidSize && contract.ask < contract.bid + 0.06) col.style.backgroundColor = "#" + rate.toString(16) + "ccf";
			...
			/////////////////////// CALLS //////////////////////////////////
				...
				for (let contractStrike in data.callExpDateMap[contractDate]) {
					...
					if (margin(contract, data) >  marginThreshold(contract)) {
						...
						if (returnrate > 19 || returnrate2 > 19) callArray.unshift(contract);
					}
				}	       
				...
				callArray.forEach(function(contract) {
					...
					if (maxReturn < returnrate || maxReturn < returnrate2) {
						...
						D('optionslist').appendChild(buildli(contract, margin(contract, data), returnrate, returnrate2, theleg, prem));
					}
				});
			...
			/////////////////////// MIDDLE //////////////////////////////////
				...
				addCol(data.symbol + " " + data.callExpDateMap[contractDate][defData.strikeName][0].daysToExpiration + " DTE", "700px", referenceli);
			...
			/////////////////////// PUTS //////////////////////////////////
				...
				for (let contractStrike in data.putExpDateMap[contractDate]) {
					...
					if (margin(contract, data) >  marginThreshold(contract)) {
						returnrate = returnRate(contract, defData);
						returnrate2 = returnRate(contract, altData);
						if (returnrate > 19 || returnrate2 > 19) putArray.push(contract);
					}
				}
