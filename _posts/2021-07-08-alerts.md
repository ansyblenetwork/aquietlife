...
<div style="margin-bottom:10px; text-align:center" id="earnings"><a target='_blank' href='https://finance.yahoo.com/calendar/earnings'>Earnings</a> within a week and weeklys</div>
<button id="earningsButton" style="margin:0px auto 20px auto; display:block;" onclick="addEarnings();">Load</button>
...
fetch("https://api.tdameritrade.com/v1/marketdata/chains?apikey=T1V8GYUYK3GKC7HG3L23O9XBJ5OH1C4F&symbol=AAPL&contractType=PUT&strikeCount=3").then(function(response) {
	...
	if (data.quoteResponse.result) {	
		let earningsString = "Earnings within a week and weeklys: ";
		...
		data.quoteResponse.result.forEach(function(stockData) {
			if (stockData.earningsTimestampStart == stockData.earningsTimestampEnd &&
				stockData.earningsTimestampEnd - Date.now()/1000 > 0 &&
				stockData.earningsTimestampEnd - Date.now()/1000 < 7*24*60*60) {
				earnings.push(stockData.symbol);
				earningsString += stockData.symbol + " ";
			}
		});
		
		D('earnings').innerHTML = earningsString;
		D('earningsButton').textContent = "Add Weeklys";
	}
	});
...
function addEarnings() {
	if (D('stButton').textContent != "Load") {
		D('tickerInput').value = earnings.join(); 
		submitTicker();
	}
}
