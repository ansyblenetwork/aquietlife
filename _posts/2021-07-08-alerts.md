		...
		if (data.status == "SUCCESS") {
			...
			for (let contractDate in data.callExpDateMap) {
				...
				for (let contractStrike in data.callExpDateMap[contractDate]) {
					...
					if (contract.bid > dpremium && lastTrade < 12*60 && returnrate > 14 && margin > 19) {
						if (!referenceli.nextElementSibling) optionslist.appendChild(buildli(contract, lastTrade, margin, returnrate, returnrate2));
						else optionslist.insertBefore(buildli(contract, lastTrade, margin, returnrate, returnrate2), referenceli.nextElementSibling);
					}
				}
			}
			...
			for (let contractDate in data.putExpDateMap) {
				...
				for (let contractStrike in data.putExpDateMap[contractDate]) {
					...
					if (contract.bid > dpremium && lastTrade < 12*60 && returnrate > 14 && margin > 10)						
						optionslist.appendChild(buildli(contract, lastTrade, margin, returnrate, returnrate2));
				}
			}
		}					       
		else D('msg').textContent = "Ticker not found.";		       
