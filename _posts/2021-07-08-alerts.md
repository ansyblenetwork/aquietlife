		...
		if (data.status == "SUCCESS") {
			...
			for (let contractDate in data.callExpDateMap) {
				...
				for (let contractStrike in data.callExpDateMap[contractDate]) {	
					...
					if (dpremium == 0 || (contract.ask > 0 && contract.ask < dpremium) || (contract.ask == dpremium && contract.strikePrice < dstrike) ) {
						
						if (apremium == 0 || astrike < dstrike) {
							astrike = dstrike;
							apremium = dpremium;
				...
				let xcontract = data.callExpDateMap[contractDate][theStrike][0];
				let xlastTrade = Math.round((Date.now() - contract.tradeTimeInLong)/60000);
				let xreturnrate = Math.round(100*contract.bid * 365 /(contract.strikePrice * contract.daysToExpiration));
				let xmargin = Math.round(100*(contract.strikePrice - data.underlyingPrice)/data.underlyingPrice);
				...
				let myli = buildli(xcontract, xlastTrade, xreturnrate, xmargin);
				myli.style.fontWeight = "bold";
								       
				for (let contractStrike in data.callExpDateMap[contractDate]) {
					let contract = data.callExpDateMap[contractDate][contractStrike][0];
					let lastTrade = Math.round((Date.now() - contract.tradeTimeInLong)/60000);
					let returnrate = Math.round(100*(contract.bid - dpremium) * 365 /((dstrike - contract.strikePrice) * contract.daysToExpiration));
