...
function updatePercent(stock, fixed) {			
	...
	if (fixed) {				
		...
		if (lpelt != document.activeElement) lpelt.value = round(10000*(current - parseFloat(lower.value))/current)/100;
		if (upelt != document.activeElement) upelt.value = round(10000*(parseFloat(upper.value) - current)/current)/100;
	}
}
