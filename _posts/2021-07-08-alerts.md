...
function updateWSB() {
	...
	fetch("https://ansyble.herokuapp.com/cors/", 
		...
		data.forEach(function(stockData) {
			let symbol = stockData.ticker;
			wsbList.push(symbol);
			if (weeklys.includes(symbol)) {
				wsbWeeklyList.push(symbol);
				wsbString += "<strong>" + symbol + "</strong> ";
				if (prevWeekly.length > 0 && !prevWeekly.includes(symbol)) {
					if (active(D('soundButton'))) D('audio').play();
		...
		data.symbols.forEach(function(stockData) {
			...
			if (weeklys.includes(stockData.symbol)) {
				...
				if (prevWeekly.length > 0 && !prevWeekly.includes(stockData.symbol)) {
					if (active(D('soundButton'))) D('audio').play();
					if (active(D('notificationButton'))) new Notification(stockData.symbol + " is newly trending on Stocktwits.");
				}
...
function toggleAlerts(elt) {
	if (yahootimer) {
		...
		wsbtimer = setInterval(function() { updateWSB(); }, 60000*5);
		yolotimer = setInterval(function() { updateYolo(); }, 60000*5);
	}
}
