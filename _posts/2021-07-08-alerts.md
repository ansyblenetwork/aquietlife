		...
		if (data.status == "SUCCESS") {
			...
			/////////////////////// CALLS //////////////////////////////////
				...
				for (let contractStrike in data.callExpDateMap[contractDate]) {
					...
					if (returnrate > 19 && margin(contract, data) > marginThreshold(contract)) {
...
function optionsChain() {	
	...
	fetch("https://api.tdameritrade.com/v1/marketdata/chains?apikey=T1V8GYUYK3GKC7HG3L23O9XBJ5OH1C4F&symbol="
		...
		if (data.status == "SUCCESS") {
			...
			function buildli(contract, margin, returnrate, returnrate2) {
				let li = make('li');
			...
			/////////////////////// CALLS //////////////////////////////////
				...
				for (let contractStrike in data.callExpDateMap[contractDate]) {
					...
					if (returnrate > 19 && margin(contract, data) >  marginThreshold(contract)) {
						if (!referenceli.nextElementSibling) D('optionslist').appendChild(buildli(contract, margin(contract, data), returnrate, returnrate2));
						else D('optionslist').insertBefore(buildli(contract, margin(contract, data), returnrate, returnrate2), 
				...
				for (let contractStrike in data.putExpDateMap[contractDate]) {
					...
					if (returnrate > 19 && margin(contract, data) >  marginThreshold(contract)) {	
						if (!referenceli.nextElementSibling) 
							D('optionslist').appendChild(buildli(contract, margin(contract, data), returnrate, returnrate2));
						else D('optionslist').insertBefore(buildli(contract, margin(contract, data), returnrate, returnrate2), 
							referenceli.nextElementSibling);
					}
				}
