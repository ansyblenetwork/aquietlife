...
<ul style="list-style-type:none; padding-left:0px;margin-bottom:20px;" id="optionslist"></ul>
...
function contractAboveVolatility(contract, data) {
	...
	return (optionsQuotes[data.symbol] && Math.sqrt(weeksToExp) * (optionsQuotes[data.symbol].volatility.split('% ')[0]) * 2 < margin(contract, data));
}
...
function generateRec(symbol) {
	...
	/////////////////////// CALLS //////////////////////////////////
		...
		recData[symbol] = {data: data};
		if (candidate) {
			recData[symbol].call = {contract:candidate, altData:altData};
			// D('recList').appendChild(buildli(candidate, data, null, altData));
		}
	...
	/////////////////////// PUTS //////////////////////////////////
		...
		if (candidate) {
			recData[symbol].put = {contract:candidate, altData:altData};
			// D('recList').appendChild(buildli(candidate, data, null, altData));
		}
		else if (!recData[symbol].call) delete recData[symbol];
							    
							    
		if (recData[symbol]) {
				if (recData[symbol].call) {
					let calldefleg2 = buildli(recData[symbol].call.altData.contract, recData[symbol].data);
					D('recList').appendChild(calldefleg2);
					referenceli = calldefleg2;	
					D('recList').appendChild(buildli(recData[symbol].call.contract, recData[symbol].data, calldefleg2, recData[symbol].call.altData));
				}
			...
			/////////////////////// MIDDLE //////////////////////////////////
				...
				addCol(recData[symbol].data.symbol + " " + recData[symbol].put.altData.contract.daysToExpiration + " DTE", "675px", referenceli);
				D('recList').appendChild(referenceli);	
	
			/////////////////////// PUTS //////////////////////////////////
					
				if (recData[symbol].put) {
					let putdefleg2 = buildli(recData[symbol].put.altData.contract, recData[symbol].data);
					D('recList').appendChild(putdefleg2);
					putdefleg2.style.marginBottom = "30px";
					D('recList').insertBefore(buildli(recData[symbol].put.contract, recData[symbol].data, putdefleg2, recData[symbol].put.altData), referenceli.nextElementSibling);			    
				}
		}
...
function updateOptions(newOption) {	
	...
	fetch("https://ansyble.herokuapp.com/cors/", 
		...
		if (optionsData[query])	generateRec(query);
		if (currentOption == query) {
			for (let i = 0; i < D('optionslist').children.length; i++) {
				if (Math.sqrt(parseFloat(D('optionslist').children[i].children[0].title)) * optionsQuotes[query].volatility.split('% ')[0] * 2 < 										parseFloat(D('optionslist').children[i].children[1].title)) {
	...
	fetch("https://api.tdameritrade.com/v1/marketdata/chains?apikey=T1V8GYUYK3GKC7HG3L23O9XBJ5OH1C4F&symbol=" 
		...
		if (data.status == "SUCCESS") {	
			...
			if (optionsQuotes[query]) generateRec(query);
			...
			function buildDefense(contract) {	
				let myli = buildli(contract, data);
				let rate = 255 - 100*16*contract.ask;								       
				if (rate < 0) myli.children[6].style.backgroundColor = "#00ffff";
				else if (rate < 16)  myli.children[6].style.backgroundColor = "#0" + rate.toString(16) + "ffff";
				else  myli.children[6].style.backgroundColor = "#" + rate.toString(16) + "ffff";
				return myli;
			}
			...
			/////////////////////// CALLS //////////////////////////////////
				...
				let calldefleg = buildDefense(defData.contract);
				D('optionslist').appendChild(calldefleg);
				let calldefleg2 = buildDefense(altData.contract);
			...
			/////////////////////// PUTS //////////////////////////////////
				...
				let putdefleg2 = buildDefense(altData.contract);
				D('optionslist').appendChild(putdefleg2);
				let putdefleg = buildDefense(defData.contract);
