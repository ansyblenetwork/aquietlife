		...
		if (data.status == "SUCCESS") {
			...
			for (let contractDate in data.callExpDateMap) {
				...
				for (let contractStrike in data.callExpDateMap[contractDate]) {	
					...
					if (contract.ask > 0) {	       
						if (contract.strikePrice < dstrike 
							&& (contract.ask < apremium || (contract.ask == apremium && contract.strikePrice < astrike))) {
							astrike = dstrike;
							apremium = dpremium;
							theaStrike = theStrike;			       
						}

						if ( contract.ask < dpremium || (contract.ask == dpremium && contract.strikePrice < dstrike)) {
				...
				function buildli(contract, lastTrade, returnrate, margin) {
					let li = make('li');
				...
				let returnrate = Math.round(100*contract.bid * 365 /(contract.strikePrice * contract.daysToExpiration));
				let margin = Math.round(100*(contract.strikePrice - data.underlyingPrice)/data.underlyingPrice);
										       
				let myli = buildli(contract, lastTrade, returnrate, margin);
				...
				returnrate = Math.round(100*contract.bid * 365 /(contract.strikePrice * contract.daysToExpiration));
				margin = Math.round(100*(contract.strikePrice - data.underlyingPrice)/data.underlyingPrice);
										       
				myli = buildli(contract, lastTrade, returnrate, margin);
				myli.style.fontWeight = "bold";
				...
				for (let contractStrike in data.callExpDateMap[contractDate]) {
					...
					if (contract.bid > 0 && lastTrade < 12*60 && returnrate > 14 && margin > 14) 
						buildli(contract, lastTrade, returnrate, margin);
				}
			}
		}					       
		else D('msg').textContent = "Ticker not found.";		       
