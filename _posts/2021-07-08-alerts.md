...
function generateRec(symbol) {
	...
	//	}
		sortRecData.sort(function(x, y) {
			...
			let ycall = 0;
			let yput = 0;
			if (x.candidateObj.call) xcall = returnRate(x.candidateObj.call.contract, recData[x.symbol].data);
			if (x.candidateObj.put) xput = returnRate(x.candidateObj.put.contract, recData[x.symbol].data);
			if (y.candidateObj.call) ycall = returnRate(y.candidateObj.call.contract, recData[y.symbol].data);
			if (y.candidateObj.put) yput = returnRate(y.candidateObj.put.contract, recData[y.symbol].data);
