...
<div style="margin-bottom:10px; text-align:center" id="earnings">Earnings within a week and weeklys</div>
	...
	if (data.quoteResponse.result) {	
		data.quoteResponse.result.forEach(function(stockData) {
			if (stockData.earningsTimestampStart == stockData.earningsTimestampEnd &&
				...
				stockData.earningsTimestampEnd - Date.now()/1000 < 7*24*60*60) earnings.push(stockData.symbol);
		});
		
		D('earnings').innerHTML = yoloString;
