		...
		if (data.status == "SUCCESS") {
			...
			/////////////////////// CALLS //////////////////////////////////
				...
				for (let contractStrike in data.callExpDateMap[contractDate]) {
					...
					if (contract.bid > defData.premium && returnrate > 19 && margin > threshold) {
	...
	fetch("https://api.tdameritrade.com/v1/marketdata/chains?apikey=T1V8GYUYK3GKC7HG3L23O9XBJ5OH1C4F&symbol="
		...
		if (data.status == "SUCCESS") {
			...
			function buildDefense(contract) {	
				...
				let myli = buildli(contract, margin, returnrate, returnrate2);
			...
			/////////////////////// CALLS //////////////////////////////////
				...
				for (let contractStrike in data.callExpDateMap[contractDate]) {
					...
					if (contract.daysToExpiration < 3) threshold = 0;  
					if (contract.bid > defData.premium && returnrate > 19 && margin > threshold) {
						if (!referenceli.nextElementSibling) D('optionslist').appendChild(buildli(contract, margin, returnrate, returnrate2));
						else D('optionslist').insertBefore(buildli(contract, margin, returnrate, returnrate2), 
				...
				for (let contractStrike in data.putExpDateMap[contractDate]) {
					...
					if (contract.bid > defData.premium && returnrate > 19 && margin > threshold) {	
						if (!referenceli.nextElementSibling) 
							D('optionslist').appendChild(buildli(contract, margin, returnrate, returnrate2));
						else D('optionslist').insertBefore(buildli(contract, margin, returnrate, returnrate2), 
							referenceli.nextElementSibling);
					}
				}
