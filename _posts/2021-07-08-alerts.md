...
function generateRec(symbol) {
	...
	/////////////////////// CALLS //////////////////////////////////
		...
		for (let contractStrike in data.callExpDateMap[contractDate]) {
			...
			if (margin(contract, data) >  marginThreshold(contract) 
				...
				&& contractAboveVolatility(contract, data)) callArray.unshift(contract);
		}	       
	...
	/////////////////////// PUTS //////////////////////////////////
		...
		for (let contractStrike in data.putExpDateMap[contractDate]) {
			...
			if (margin(contract, data) >  marginThreshold(contract)
				&& returnRate(contract, altData) > 19 
				&& contractAboveVolatility(contract, data)) putArray.push(contract);
		}
	...
	fetch("https://api.tdameritrade.com/v1/marketdata/chains?apikey=T1V8GYUYK3GKC7HG3L23O9XBJ5OH1C4F&symbol=" 
		...
		if (data.status == "SUCCESS") {	
			...
			function buildDefense(contract) {	
				...
				let myli = buildli(contract, data, returnrate);
			...
			function buildli(contract, data, returnrate, defleg, altData) {
				let li = make('li');
				...
				let colstring = Math.round(returnrate) + '/' + Math.round(returnRate(contract, altData)) + "%";
			...
			/////////////////////// CALLS //////////////////////////////////
				...
				callArray.forEach(function(contract) {
					...
					if (maxReturn < returnrate2) {
						maxReturn = returnrate2;
						D('optionslist').appendChild(buildli(contract, data, returnrate, calldefleg2, altData));
					}
				});
			...
			/////////////////////// PUTS //////////////////////////////////
				...
				putArray.forEach(function(contract) {
					...
					if (maxReturn < returnrate2) {
						maxReturn = returnrate2;
						D('optionslist').insertBefore(buildli(contract, data, returnrate, putdefleg2, altData), 
