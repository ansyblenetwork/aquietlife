...
function optionsChain() {
	if (currentOption != D('optionsInput').value.toUpperCase()) {
		if (tickerList.includes(currentOption)) {
			tickerList.splice(tickerList.indexOf(currentOption), 1); 
			delete volumes[currentOption];
			delete prices[currentOption];
		}
		currentOption = D('optionsInput').value.toUpperCase();
		D('tickerInput').value = currentOption;
		submitTicker(D('optionsPrice'));
	}
...
function submitTicker(elt) {
	...
	for (let i = data.quoteResponse.result.length - 1; i > -1; i--) {
		...
		if (!tickerList.includes(stock)) {			
			...
			if (elt) { elt.textContent = ""; elt.appendChild(newli); }
