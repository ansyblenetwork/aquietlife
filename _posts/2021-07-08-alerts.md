	...
	fetch("https://api.tdameritrade.com/v1/marketdata/chains?apikey=T1V8GYUYK3GKC7HG3L23O9XBJ5OH1C4F&symbol="
		...
		if (data.status == "SUCCESS") {
			...
			function buildli(contract, lastTrade, margin, returnrate, returnrate2) {
				...
				addCol(contract.bid + ' x ' + contract.bidSize, "100px");
				addCol(contract.ask + ' x ' + contract.askSize, "100px");
				addCol(contract.last + ' x ' + contract.lastSize, "100px");	
			...
			for (let contractDate in data.callExpDateMap) {
				...
				let lastTrade = Math.round((Date.now() - contract.tradeTimeInLong)/60000);
				let returnrate = "x";
				let returnrate2 = "x";
				...
				myli.style.paddingTop = "10px";
	
				if (theaStrike) {					       
					contract = data.callExpDateMap[contractDate][theaStrike][0];
					lastTrade = Math.round((Date.now() - contract.tradeTimeInLong)/60000);
				...
				for (let contractStrike in data.callExpDateMap[contractDate]) {
					...
					lastTrade = Math.round((Date.now() - contract.tradeTimeInLong)/60000);
					returnrate = Math.round(100*(contract.bid - dpremium) * 365 /((dstrike - contract.strikePrice) * contract.daysToExpiration));
					if (returnrate == 46) {
						console.log(contract.bid);
						console.log(dpremium);
						console.log(dstrike);
						console.log(contract.strikePrice);
						console.log(contract.daysToExpiration);
					}
					returnrate2 = Math.round(100*(contract.bid - apremium) * 365 /((astrike - contract.strikePrice) * contract.daysToExpiration));
					margin = Math.round(100*(contract.strikePrice - data.underlyingPrice)/data.underlyingPrice);
								  
					if (contract.bid > dpremium && lastTrade < 12*60 && returnrate > 14 && margin > 19) 
						buildli(contract, lastTrade, margin, returnrate, returnrate2);
				}
			}
			...
			for (let contractDate in data.putExpDateMap) {
				...
				for (let contractStrike in data.putExpDateMap[contractDate]) {	
					...
					if (contract.ask > 0) {	
						console.log("trying a put update");
						console.log("contract.ask: " + contract.ask);
						console.log("dpremium: " + dpremium);
						console.log("apremium: " + apremium);
						console.log("dstrike: " + dstrike);
						console.log("astrike: " + astrike);
						console.log((contract.ask > dpremium 
							&& (apremium == 0 || contract.ask < apremium || (contract.ask == apremium && contract.strikePrice > astrike))));
						if (contract.ask < dpremium
							&& (apremium == 0 || dpremium < apremium || (dpremium == apremium && dstrike > astrike))) {
							astrike = dstrike;
							...
							console.log("pushing to aprem the dstrike");
						} else if (contract.ask > dpremium 
							&& (apremium == 0 || contract.ask < apremium || (contract.ask == apremium && contract.strikePrice > astrike))) {
							astrike = contract.strikePrice;
							apremium = contract.ask;
							...
							console.log("pushing to aprem the new candidate");		       
						}
						if (dpremium == 0 || contract.ask < dpremium || (contract.ask == dpremium && contract.strikePrice > dstrike)) {
							dstrike = contract.strikePrice;
							dpremium = contract.ask;
				...
				for (let contractStrike in data.putExpDateMap[contractDate]) {
					...
					lastTrade = Math.round((Date.now() - contract.tradeTimeInLong)/60000);
					returnrate = Math.round(100*(contract.bid - dpremium) * 365 /((contract.strikePrice - dstrike) * contract.daysToExpiration));
					returnrate2 = Math.round(100*(contract.bid - apremium) * 365 /((contract.strikePrice - astrike) * contract.daysToExpiration));
					margin = Math.round(100*(data.underlyingPrice - contract.strikePrice)/data.underlyingPrice);
