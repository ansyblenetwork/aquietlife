...
function submitTicker() {
	...
	if (buffer) {	
		...
		if (!tickerList.includes(stock)) {
			...
			let current = make("span");
			current.style.padding = "10px";
			current.textContent = buffer.regularMarketPrice;
	
			let uuperBound = make("input");
			uuperBound.value = Math.floor(buffer.regularMarketPrice * 1.1*100)/100;
	
			newli.appendChild(newTickerX);
			newli.appendChild(newTicker);
			newli.appendChild(lowerBound);
			newli.appendChild(current);
			newli.appendChild(uuperBound);
